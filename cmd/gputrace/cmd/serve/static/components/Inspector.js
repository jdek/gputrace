...
const ResourceSection = ({ title, resources, onSelect }) => {
...
export const Inspector = ({ item, onSelect }) => {
  if (!item) {
    return html`
  ...
  const renderIcon = () => {
       switch(item.type) {
           case NodeType.ENCODER: return html`<${Cpu} className="text-orange-500" size=${20}/>`;
           case NodeType.DISPATCH: return html`<${Code} className="text-blue-500" size=${20}/>`;
           case NodeType.BARRIER: return html`<${XCircle} className="text-red-500" size=${20}/>`;
           case NodeType.BUFFER: return html`<${Database} className="text-green-500" size=${20}/>`;
           case NodeType.ROOT: return html`<${FileText} className="text-purple-500" size=${20}/>`;
           default: return html`<${Info} className="text-gray-400" size=${20}/>`;
       }
  }
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        ...
        <div className="mb-6">
            ...
            <div className="bg-[#252526] rounded-md border border-gray-700 p-3 space-y-0.5">
                <${InfoRow} label="Type" value=${html`<span className="capitalize text-blue-300 bg-blue-900/20 px-1 rounded text-[10px]">${item.type}</span>`} />
                <${InfoRow} label="ID" value=${html`<span className="text-[10px] text-gray-500">${item.id}</span>`} />
                ${item.stats?.duration && html`<${InfoRow} label="Duration" value=${item.stats.duration} />`}
                ${item.stats?.threads && html`<${InfoRow} label="Threads" value=${item.stats.threads} />`}
                ${item.properties && Object.entries(item.properties).map(([key, val]) => html`
